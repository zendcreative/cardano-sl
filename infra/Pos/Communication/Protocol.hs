...
import           System.Wlog (LoggerName, usingLoggerName, logWarning)
...
makeEnqueueMsg
    :: LoggerName
    -> VerInfo
    -> (forall t . Msg -> (NodeId -> VerInfo -> N.Conversation PackingType t) -> IO (Map NodeId (IO t)))
    -> EnqueueMsg
makeEnqueueMsg lname ourVerInfo enqueue = \msg mkConv -> enqueue msg $ \nodeId pVI ->
    alternativeConversations lname nodeId ourVerInfo pVI (mkConv nodeId pVI)
...
alternativeConversations lname nid ourVerInfo theirVerInfo convs
  ...
  where
    ...
    throwErrs errs (Conversation l) = N.Conversation $ \conv -> do
        let _ = l conv
        usingLoggerName lname $ logWarning $ sformat
    ...
    logOSNR (Right e@(OutSpecNotReported _ _)) = usingLoggerName lname $ logWarning $ sformat build e
    logOSNR _                                  = pure ()
...
makeSendActions
    ...
    -> SendActions
makeSendActions lname ourVerInfo enqueue converse = SendActions
    { withConnectionTo = \nodeId mkConv -> N.converseWith converse nodeId $ \pVI ->
          alternativeConversations lname nodeId ourVerInfo pVI (mkConv pVI)
    , enqueueMsg = makeEnqueueMsg lname ourVerInfo enqueue
    }
...
checkingInSpecs ourVerInfo peerVerInfo' spec nodeId action =
    if | spec `notInSpecs` vIInHandlers ourVerInfo ->
              usingLoggerName "checkingInSpecs" $ logWarning $ sformat
